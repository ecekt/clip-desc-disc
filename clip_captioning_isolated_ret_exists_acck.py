...
if not os.path.exists('results/clip_captionword_GOLD_ISOLATED_RETEXISTS_ACCK'):
    os.mkdir('results/clip_captionword_GOLD_ISOLATED_RETEXISTS_ACCK')
...
def closest_word(text_encs, vis_enc):
    ...
    index = [n.item() for n in torch.topk(scores.squeeze(), k=accuracy_at_k).indices]

    return index

with open('full_pb_clip_captionword_GOLD_ISOLATED_RETEXISTS_ACCK.txt', 'w') as f:
    ...
    for ch in full_chains:
        ...
        for utt in ch['utterances']:
            ...
            with torch.no_grad():
                ...
                index_closest = closest_word(text, difference_features)
            ...
            predicted_words = [i2w[ic] for ic in index_closest]
            ...
            for predicted_word in predicted_words:
                ...
                if predicted_word.lower() in tweet_tokenizer.tokenize(utt_str):
...
torch.save(chain_rank_scores, 'results/clip_captionword_GOLD_ISOLATED_RETEXISTS_ACCK/full_pb_clip_captionword_GOLD_ISOLATED_RETEXISTS_ACCK_chain_rank_scores.pt')
